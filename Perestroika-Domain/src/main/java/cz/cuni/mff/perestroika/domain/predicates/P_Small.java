...
public final class P_Small extends Predicate {
	...
	public static int toInt(T_Location l) {
		return (T_Location.getIndex(l) << (Predicate.MASK_TYPE_BIT_COUNT)) | FLAG_TYPE;
	}
	...
	public static final class Storage_P_Small implements IStorage<P_Small> {
		...
		/**
		 * Returns TRUE if a NEW predicate was set; false if the predicate was already
		 * stored.
		 * 
		 * @param v
		 * @param l
		...
		/**
		 * Returns TRUE if a predicate was removed (i.e. was previously set within the
		 * storage)
		 * 
		 * @param v
		 * @param l
